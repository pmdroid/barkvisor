describe('Disk Management', () => {
  ...
  it('opens Create Disk modal with name, size, format fields', () => {
    ...
    cy.get('.split-frame input[placeholder="data-disk"]').should('exist')
    cy.get('.split-frame input[type="number"]').should('exist')
    cy.get('.split-frame select').should('exist')
    cy.get('.split-frame select option').should('contain', 'QCOW2')
  })

  it('Create Disk offers QCOW2 and Raw format options', () => {
    cy.contains('button', 'Create Disk').click()
    cy.get('.split-frame select option').should('contain', 'QCOW2')
    cy.get('.split-frame select option').should('contain', 'Raw')
    cy.get('.split-frame').contains('button', 'Cancel').click()
  })

  it('create disk validates name is required', () => {
    cy.contains('button', 'Create Disk').click()
    cy.get('.split-frame').contains('button', 'Create').click()
  ...
  it('creates a new QCOW2 disk', () => {
    cy.contains('button', 'Create Disk').click()
    cy.get('.split-frame input[placeholder="data-disk"]').type(diskName)
    cy.get('.split-frame input[type="number"]').clear().type('1')
    cy.get('.split-frame').contains('button', 'Create').click()
    cy.get('.modal-overlay').should('not.exist')
