...
describe('homeLibrary store (PAS-34)', () => {
  ...
  test('unions ready images across Devices by checksum and tracks local copies', async () => {
    ...
    expect(store.images.find((row) => row.name === 'ubuntu.iso')?.sourceHostIds.sort()).toEqual(['desk', 'studio'])
    expect(store.images.find((row) => row.name === 'extra.iso')?.sourceHostIds).toEqual(['desk'])
  ...
  test('non-ready copies stay in the library but are not deployable', async () => {
    ...
    expect(row?.sourceHostIds).toEqual(['desk'])
