...
const appMetrics = ref<MetricSample[]>([])
let appMetricsTimer: number | undefined
let appMetricsEpoch = 0

const appMetricsPath = computed(() => {
  if (!vm.value || !isApp.value || vm.value.state !== 'running') return null
  if (isMemberDetail.value) {
    const device = memberDevice.value
    if (!device || !canFetchDeviceWorkloads(device)) return null
    return deviceVmMetricsPath(device, vmId.value)
  }
  return `/vms/${encodeURIComponent(vmId.value)}/metrics`
})

async function fetchAppMetrics(epoch: number) {
  const path = appMetricsPath.value
  if (!path) {
    if (epoch === appMetricsEpoch) appMetrics.value = []
    return
  }
  try {
    const { data } = await api.get<MetricSample[]>(path, { params: { minutes: 30 } })
    if (epoch === appMetricsEpoch && Array.isArray(data)) appMetrics.value = data
  } catch {
    if (epoch === appMetricsEpoch) appMetrics.value = []
  }
}

watch(appMetricsPath, (path) => {
  appMetricsEpoch++
  const epoch = appMetricsEpoch
  if (appMetricsTimer) {
    clearInterval(appMetricsTimer)
    appMetricsTimer = undefined
  }
  if (!path) {
    appMetrics.value = []
    return
  }
  void fetchAppMetrics(epoch)
  appMetricsTimer = window.setInterval(() => {
    void fetchAppMetrics(epoch)
  }, 5000)
}, { immediate: true })
