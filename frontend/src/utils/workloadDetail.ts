...
import { isApplicationWorkload } from './workloadKind'
...
export type WorkloadOpenTarget = {
  hostId: string
  role?: string | null
  vm: { id: string } & Partial<Pick<VM, 'kind' | 'spec'>>
}
...
export function workloadDetailPath(row: WorkloadOpenTarget): string {
  const collection = isApplicationWorkload(row.vm) ? 'apps' : 'vms'
  if (!row.hostId || row.role === 'self') {
    return `/${collection}/${encodeURIComponent(row.vm.id)}`
  }
  return `/devices/${encodeURIComponent(row.hostId)}/${collection}/${encodeURIComponent(row.vm.id)}`
}
