...
describe('PAS-82 Home terminology', () => {
  ...
  test('Device drill-down lists Workloads and can open Create on that Device', () => {
    ...
    expect(text).toContain('saveDeviceName')
    ...
    expect(text).toContain('terminal-modal')
    expect(text).toContain('modal-overlay')
    expect(text).toContain('max-height: none')
    expect(text).toContain('align-items: stretch')
    expect(text).not.toContain('min(1120px')
    expect(text).not.toContain('calc(100vw - 32px)')
    expect(text).not.toContain('terminal-sheet')
