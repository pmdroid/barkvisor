...
Prefer **Networks → Bridge setup → Device address (DHCP/static) → Apply/Revert** on the Device. Equivalent commands stay on that page. Rollback is a host timer (`netplan try`). Do not Confirm in the browser after the uplink dies. Wi-Fi is refused.
