...
**Terminal** on the Device detail toolbar opens a shell in the browser as a login account on that Device. Pick the account, confirm, then type. BarkVisor admin is the credential; there is no OS password prompt. Root is not offered. Further privilege is whatever `sudo` already allows that account.
