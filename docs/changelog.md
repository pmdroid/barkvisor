...
- Device stats history includes **GPU** busy percent (`hostGpuPercent`) on Mac (IOAccelerator) and Linux (`gpu_busy_percent` / i915 RC6). Device detail shows CPU, memory, and GPU. The Ollama page shows GPU only. Occupancy / passthrough is unchanged; i915 is not unbound.
