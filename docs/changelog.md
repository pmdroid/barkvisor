...
- Library **Onyx** (Lite): one-click Workload, Agent cage, Home Ollama at `http://10.0.2.2:11434` (native, not Device `:7777`). Guest UI on `:80` (loopback hostfwd). In-app Chat tab is gone; completions stay on `/v1/chat/completions`.
