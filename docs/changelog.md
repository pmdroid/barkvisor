...
- Home hop errors no longer all say **Device is unreachable**. A connect timeout, cancel, or TLS failure is **Home cannot hop**; a member HTTP 5xx is the Device answering badly (Ollama down on that hop); only a failed health probe is offline. The same codes land on `/api/home/devices/health` `reachability` / `reachabilityError`.
