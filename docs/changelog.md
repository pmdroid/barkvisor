...
- Bridged start on Linux denies when qemu-bridge.conf is missing or unreadable (same as the Networks UI). macOS no longer falls back to a shared socket_vmnet socket; the per-interface daemon socket is required.
