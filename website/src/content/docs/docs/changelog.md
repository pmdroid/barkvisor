...
- Linux Create Disk and Workload start fail closed when this Device cannot open a host block device for read/write. The error names the path and that the BarkVisor user needs the disk group (or a udev ACL). QEMU is not started with that `-drive`. The systemd unit grants **disk** as well as kvm (`SupplementaryGroups=kvm disk`); postinst adds the service user to group disk. Without that, QEMU `-drive file=/dev/sdX` is Permission denied.
