...
- In-app updates are back on a root appliance: Ubuntu/Debian `.deb` and Apple Silicon `.pkg`. Settings → Updates and `/api/system/updates` fail closed unless this Device is root with `/var/lib/barkvisor`. The daemon runs `dpkg -i` / `installer -pkg`; never Homebrew. Checksum or download failure refuses the apply. The SPA polls `/api/health` after restart.
