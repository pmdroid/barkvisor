...
After Apply, the host keeps changes **pending** for 60 seconds. A modal asks you to click **Keep changes** or the host auto-reverts (netplan try / systemd timer). If the NIC carries SSH or the SPA, Apply warns and asks you to confirm **before** the uplink moves.
