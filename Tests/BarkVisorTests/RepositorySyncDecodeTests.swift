...
struct RepositorySyncDecodeTests {
    @Test func `catalog fixture missing TemplateInput field names the field in lastError`() async throws {
        let json = """
        {
          "version": 1,
          "templates": [
        ...
        do {
            try await service.syncCatalogData(data, repositoryID: repoId)
            Issue.record("expected catalog decode to fail")
        } catch {
            #expect(error is BarkVisorError)
        }
    ...
    @Test func `template catalog without images still syncs`() throws {
        ...
        """
        let catalog = try RepositoryCatalogDecoder.decode(Data(json.utf8), repoName: "local")
    ...
    @Test func `repo catalog still decodes with images`() throws {
        ...
        """
        let catalog = try RepositoryCatalogDecoder.decode(Data(json.utf8), repoName: "ignored")
