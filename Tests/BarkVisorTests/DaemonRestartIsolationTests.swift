...
struct DaemonRestartIsolationTests {
    ...
    @Test func `systemd unit only kills the daemon process`() throws {
        ...
        #expect(postinst.contains("barkvisor-daemon.service barkvisor-agent.service"))
        #expect(postinst.contains("${unit}.d/disk.conf"))
        #expect(postinst.contains("SupplementaryGroups=disk"))
        #expect(postinst.contains("try-restart barkvisor-daemon.service"))
        #expect(postinst.contains("try-restart barkvisor-server.service"))
