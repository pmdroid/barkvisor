...
struct CapabilityDetailTests {
    ...
    @Test func `currentCapabilities projects details and runnable arches`() {
        ...
        #elseif os(Windows)
            #expect(caps.platform == "Windows")
            #expect(byCode[.bridgedNetworking]?.supported == false)
            #expect(byCode[.bridgedNetworking]?.reasonCode == CapabilityReasonCode.osUnsupported.rawValue)
            #expect(byCode[.usbPassthrough]?.supported == false)
            #expect(byCode[.usbPassthrough]?.reasonCode == CapabilityReasonCode.osUnsupported.rawValue)
            #expect(byCode[.kvmDevice]?.supported == false)
