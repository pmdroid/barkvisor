...
struct OllamaCatalogTests {
    @Test func `merge tags and ps marks running and keeps pulled`() {
        ...
        let running = [
            OllamaRunningRecord(name: "llama3:latest", digest: "sha-a", size: 4_000),
        ]
    ...
    @Test func `home map drops unreachable and stale devices`() {
        ...
        let snapshots = [
            OllamaDeviceSnapshot(
                ...
                probedAt: fresh,
                models: [
                    OllamaLocalModel(name: "llama3:latest", size: 100, running: true),
                ],
