...
struct CreateWorkloadTests {
    ...
    @Test func `coding agent image defaults to agent class and device ollama`() throws {
        ...
        #expect(body.cloudInit?.userData?.contains("/etc/git-hooks/pre-push") == true)
        #expect(body.cloudInit?.userData?.contains("/var/lib/barkvisor/last-git-push") == true)
    ...
    private func bridgedNetwork(id: String = "net-br") -> NetworkRecord {
        NetworkRecord(id: id, name: "LAN", mode: "bridged", bridge: "br0", isDefault: false)
    }
